...
CREATE TABLE IF NOT EXISTS `mydb`.`Actuadores` (
  ...
  CONSTRAINT `actuador_program_fk`
    ...
    ON DELETE SET NULL
    ON UPDATE CASCADE,
  CONSTRAINT `actuador_sensor_fk`
    FOREIGN KEY (`flowmeter`)
    REFERENCES `mydb`.`Sensores` (`id`)
    ON DELETE SET NULL
    ON UPDATE SET NULL)
ENGINE = InnoDB;
